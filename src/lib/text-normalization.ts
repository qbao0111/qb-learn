 ...
 * in the PDF text layer (for example "giá tri@" instead of "giá trị").
 * Restore only vowel+@ sequences and leave real email addresses untouched.
 */
export function restoreVietnamesePdfDiacritics(value = '') {
  return String(value).replace(/\S*@\S*/g, (token) => {
    ...
    return token.replace(
      ...
      (_, vowel: string) => PDF_DOT_BELOW_MAP[vowel] || `${vowel}@`,
    );
  });
}
